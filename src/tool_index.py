...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_documents": "List, read, delete, or tidy documents in the editor panel. Always pass an explicit `action`; a payload that names a document without one is rejected (ambiguous between read and delete). action='list' returns clickable rows (most-recent first) so the user can open any doc by clicking. action='read' (aka view/open/get) with document_id returns the content; supports offset=<N> + limit=<N> to page through large docs (response includes next_offset when more remains, so you can keep calling with offset=next_offset). action='delete' requires an explicit document_id — no id, no deletion. Use this for ANY 'show/read/list/open my documents/docs/files/notes' request — never shell or curl.",
    ...
    "read_email": "Read the full content of a specific email by UID or Message-ID — the uid (or message_id) is REQUIRED and a call with neither is rejected before any mailbox is contacted; call list_emails or search_emails first to get UIDs. Supports account from list_email_accounts when the UID belongs to a non-default mailbox.",
